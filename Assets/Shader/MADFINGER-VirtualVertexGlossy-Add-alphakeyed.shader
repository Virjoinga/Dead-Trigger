Ê%Shader "MADFINGER/Environment/Virtual Gloss Per-Vertex Additive AlphaKeyed (Supports Lightmap)" {
Properties {
...
void main ()
{
  ...
  highp vec4 tmpvar_4;
  tmpvar_4 = (((_SpecColor * pow (clamp (dot ((tmpvar_2 * normalize(_glesNormal)), normalize(((vec3(0.0, 0.0, 1.0) + normalize(-(tmpvar_3))) * 0.5))), 0.0, 1.0), (_Shininess * 128.0))) * _SpecStrength) * (1.0 - clamp ((sqrt(dot (tmpvar_3, tmpvar_3)) / _SpecRange), 0.0, 1.0))).xyzz;
  tmpvar_1 = tmpvar_4;
...
void main ()
{
  ...
  c_1.w = tmpvar_2.w;
  c_1.xyz = (tmpvar_2.xyz + (xlv_TEXCOORD2.xyz * tmpvar_2.w));
