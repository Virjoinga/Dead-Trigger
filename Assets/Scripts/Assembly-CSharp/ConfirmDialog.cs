public class ConfirmDialog : BasePopupScreen
{
	...
	private GUIBase_TextArea m_Message;
	...
	protected override void OnGUI_Init()
	{
		...
		{
			base.OnGUI_Init();
			m_ScreenPivot = MFGuiManager.Instance.GetPivot("Confirm_Dialog");
			m_ScreenLayout = m_ScreenPivot.GetLayout("Confirm_Layout");
			PrepareButton(m_ScreenLayout, "OK_Button", null, Delegate_OK);
			PrepareButton(m_ScreenLayout, "Cancel_Button", null, Delegate_Cancel);
			m_Caption = PrepareLabel(m_ScreenLayout, "Caption_Label");
			m_Message = PrepareTextArea(m_ScreenLayout, "Text_Label");
