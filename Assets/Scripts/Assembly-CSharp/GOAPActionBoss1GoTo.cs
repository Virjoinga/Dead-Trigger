...
internal class GOAPActionBoss1GoTo : GOAPAction
{
	...
	public GOAPActionBoss1GoTo(AgentHuman owner)
		: base(E_GOAPAction.Boss1Goto, owner)
	{
	}
	...
	public override void Activate()
	{
		...
		SetMotionType();
		Owner.StartCoroutine(_DoSetInvulnerable((!RunLikeHell) ? 4.5f : 3f));
	...
	public override void Update()
	{
		if (!Owner.BlackBoard.ActionPointOn && (bool)Owner.BlackBoard.DangerousEnemy)
		{
			...
			if (Time.timeSinceLevelLoad > NextLookAroundTime && Owner.BlackBoard.Fear > 50f)
			{
				...
				RunLikeHell = true;
				Owner.BlackBoard.SetFear(0f);
				Owner.StartCoroutine(_DoPlayAnim("BossIdle"));
			}
			else
			{
