...
public class AnimStateAttackVomitPresent : AnimState
{
	...
	private ComponentBody BodyComponent;
	...
	private bool ThrowRightHand = true;

	public AnimStateAttackVomitPresent(Animation anims, AgentHuman owner)
		: base(anims, owner)
	{
	}
	...
	{
		EnemyComponent.PlayAttackSound(E_WeaponAction.ThrowRight);
		...
		projectileInitSettings.Speed = 17f;
		...
		projectileInitSettings.Destination = Owner.BlackBoard.DangerousEnemy.Position;
		Vector3 vector = ((!ThrowRightHand) ? BodyComponent.LeftHand.position : BodyComponent.RightHand.position);
		Vector3 inDir = projectileInitSettings.Destination - vector;
		projectileInitSettings.EffectiveRange = inDir.magnitude;
		inDir.y += projectileInitSettings.EffectiveRange;
		ProjectileManager.Instance.SpawnProjectile(ProjectileType, vector, inDir, projectileInitSettings);
	}
	...
	private void PlayAnim()
	{
		...
		string weaponAnim = Owner.AnimSet.GetWeaponAnim((!ThrowRightHand) ? E_WeaponAction.ThrowLeft : E_WeaponAction.ThrowRight);
	...
	protected override void Initialize(AgentAction action)
	{
		...
		BodyComponent = Owner.GetComponent<ComponentBody>();
		if (BodyComponent == null)
		{
			throw new MemberAccessException("ComponentBody not found!");
		}
