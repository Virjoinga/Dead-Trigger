...
public class StaticMesh : Editor
{
    ...
    public static void SplitScene()
    {
        ...
        if (target == null)
        {
            Debug.LogError("no combined mesh in scene");
            return;
        }

        Mesh[] splitMeshes = MeshManager.SplitMesh(target);
        
        List<Mesh> meshesToCreate = new List<Mesh>();
        ...
        {
            meshesToCreate[i].RecalculateBounds();
            meshesToCreate[i].RecalculateNormals();
            meshesToCreate[i].UploadMeshData(true);

            meshFilters[i].sharedMesh = meshesToCreate[i];
            AssetDatabase.CreateAsset(meshesToCreate[i], "Assets/StaticMesh/" + EditorSceneManager.GetActiveScene().name + "/" + meshesToCreate[i].name + ".asset");
        }
...
public static class MeshManager
{
    public static Mesh CombineMesh(Mesh[] subMeshes)
    {
        ...
        for (int i = 0; i < subMeshes.Length; i++)
        {
            mesh.SetTriangles(subMeshes[i].triangles, i, false, vertexCount);
    ...
    public static Mesh[] SplitMesh(Mesh input)
    {
        ...
        for (int i = 0; i < meshes.Length; i++)
        {
            ...
            int baseVertex = triangles.Min();

            int vertexCount = triangles.Max() + 1 - baseVertex;

            for (int j = 0; j < triangles.Length; j++)
            {
                triangles[j] -= baseVertex;
            }

            if (input.vertices != null && input.vertices.Length > 0)
            {
                meshes[i].vertices = input.vertices.Skip(baseVertex).Take(vertexCount).ToArray();
            }

            if (input.normals != null && input.normals.Length > 0)
            {
                meshes[i].normals = input.normals.Skip(baseVertex).Take(vertexCount).ToArray();
            }

            if (input.tangents != null && input.tangents.Length > 0)
            {
                meshes[i].tangents = input.tangents.Skip(baseVertex).Take(vertexCount).ToArray();
            }

            if (input.uv != null && input.uv.Length > 0)
            {
                meshes[i].uv = input.uv.Skip(baseVertex).Take(vertexCount).ToArray();
            }
            if (input.uv2 != null && input.uv2.Length > 0)
            {
                meshes[i].uv2 = input.uv2.Skip(baseVertex).Take(vertexCount).ToArray();
            }
            if (input.uv3 != null && input.uv3.Length > 0)
            {
                meshes[i].uv3 = input.uv3.Skip(baseVertex).Take(vertexCount).ToArray();
            }
            if (input.uv4 != null && input.uv4.Length > 0)
            {
                meshes[i].uv4 = input.uv4.Skip(baseVertex).Take(vertexCount).ToArray();
            }

            if (input.colors != null && input.colors.Length > 0)
            {
                meshes[i].colors = input.colors.Skip(baseVertex).Take(vertexCount).ToArray();
            }
            if (input.colors32 != null && input.colors32.Length > 0)
            {
                meshes[i].colors32 = input.colors32.Skip(baseVertex).Take(vertexCount).ToArray();
            }
